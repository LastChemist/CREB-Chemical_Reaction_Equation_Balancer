...
Parsing a simple chemical equation: H2O + CO2 → H2CO3
Counting the occurrences of each element: H(2)O + C(1)O(2) → H(2)C(1)O(3)
Mapping elements to their counts: H(2)O + C(1)O(2) → {H: 2, C: 1, O: 3}
"""
...
class EquationUtil:
    ...
    class ElementMapper:
        ...
        def search(self):
            ...
            And so, the developer continued their journey in the land of programming, armed with new knowledge and experience. They knew that no matter how tough the problems might get, they could always find a solution if they kept trying and learning. And they lived happily ever after, coding away into the sunset. 😊
            """
    ...
    class ElementCounter:
        ...
        def mapElements(self) -> None:
            ...
            # https://pylint.readthedocs.io/en/latest/user_guide/messages/convention/consider-using-enumerate.html
                ...
                while True:
                    try:
                        if (
                            ...
                            i += 1
                            continue
                    # TODO: do not use bare-exceptions
                    # https://pylint.readthedocs.io/en/latest/user_guide/messages/warning/bare-except.html
                    except:
                        multiplier = self.chemical_formula[
                            # TODO: incompatible types in assignment
                            # https://mypy.readthedocs.io/en/latest/error_code_list.html#code-assignment
    ...
    class LinearEquationSystemGenerator:
        def __init__(self, chemical_equation: str) -> None:
            ...
            # TODO: using type "object" does not help, it should be an "EquationParser"
        ...
        def presentElementsInReaction(self) -> list:
            ...
            # TODO: no return statement, users expect a "list"
    ...
    class LinearEquationSystemSolverFileMaker:
        ...
        def generateEquationAndSaveSolverFile(self):
            ...
            # TODO: even though it looks genius and satisfying to make a program write another program for you, it should be left as a last-resort. You might be able to create a some function that will handle the situation.
...
with open(r"{self.current_directory}\solution.automatic.pkl","wb") as solution_file:
    ...
    def execute(self, input_chemical_equation: str) -> None:
        # TODO: the variable below is of type "LinearEquationSystemSolverFileMaker", typing as "object" is misleading.
...
# TODO: it would be great if you could add a few "test" files using something like "pytest", to show how CREAB works, and also, check if it still working as expected after each change.
# example usage :
